    ...
    <div class="card card-default chat-box">
        <div class="card-header bg-dark text-light">
            <b :class="{'text-danger':session.block}">
                {{friend.name}}
                <span v-if="isTyping"> пишет сообщение . . .</span>
            ...
            <a href="" @click.prevent="close">
                <i class="fa fa-times float-right text-light mt-1" aria-hidden="true"></i>
            </a>
            <!-- Close Button End-->
            ...
            <div class="dropdown float-right mr-4" >
                ...
                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                    <a class="dropdown-item" href="#" v-if="session.block && can"
                       @click.prevent="unblock">Разблокировать
                    </a>
                    <a class="dropdown-item" href="#" @click.prevent="block" v-if="!session.block">Блокировать</a>


                    <a class="dropdown-item" href="#" @click.prevent="clear">Очистить переписку</a>
        ...
        <div class="card-body" v-chat-scroll>
            ...
            'text-success':chat.read_at!=null}" v-for="chat in chats"
               :key="chat.id">
                {{chat.message}}
                <br>
        ...
        <form class="card-footer" @submit.prevent="send">
            <div class="form-group">
                <input type="text" class="form-control" placeholder="Пишите Ваше сообщение тут"
                       :disabled="session.block" v-model="message">
                <br>
                <button class="float-right btn-dark" @submit.prevent="send"
                        :disabled="session.block">Отправить</button>
            </div>
